...
#Initialize a hash to allow lookup of a codon to get the corresponding
#amino acid.
...
#Initialize the variable to hold user input
my $codon = 'GCT';
while ($codon) {
	print 'Enter a codon', "\n";
	...
	#Remove end­of­line
	chomp ($codon);
	#Make sure it's initialized before trying to print
	print $codonTranslation{$codon}, "\n";
	if($codonTranslation{$codon}){
		print $codonTranslation{$codon}, "\n";
	}
}
